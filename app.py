...
if __name__=="__main__":
    ...
    word_tokenize_cfg={
        ...
        "input_file":"your_input_dataframe_or_file_path", # 输入文件，必选项，应该是一个pandas.DataFrame或内容是一个DataFrame的pickle文件的路径名
        "tokenize_column_name":"your_tokenize_column_name", # 需要执行分词的列的列名，必选项，应该是字符串格式
        "tokenized_column_name":"your_tokenized_column_name", # 分词结果的列的列名，必选项，应该是字符串格式，当且仅当only_retain_meaningful_words为True时生效
        "user_added_stop_words_filename":"path_for_user_added_stop_words", # 用户定义的停用词词表的路径名，可选项，默认为None即只根据词性进行筛选，，当且仅当only_retain_meaningful_words为True时生效，应该是一个以utf-8格式定义的每行一个词的文本文件的文件名
        "user_added_critic_words_filename":"path_for_user_added_critic_words", # 用户定义的关键词词表的路径名，可选项，默认为None，用于计算关键词的词频，，当且仅当only_retain_meaningful_words为True时生效，应该是一个以utf-8格式定义的每行一个词的文本文件的文件名
        "user_added_other_words_filename":"path_for_user_added_other_words", # 用户定义的其他词词表的路径名，可选项，默认为None，用于在分词程序未能正确识别特定词语的情况下将该词语加入词库以完成识别，应该是一个以utf-8格式定义的每行一个词的文本文件的文件名
        ...
        "output_filename":"your_output_filename.xlsx/pkl", # 输出文件名，可选项，默认为None即不需要输出，为防止文本中逗号与逗号分隔符混淆不支持csv，支持xlsx，数据量较大时会以1000000行为分割输出多个excel文件，支持pickle(.pkl)格式，输出文件会出现在finalresults文件夹中
