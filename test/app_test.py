...
if __name__=="__main__":
    ...
    # 分词配置文件
    word_tokenize_cfg={
        "runtime_code":"cyj", # 运行时代码，必选项，用于区分不同的运行时，用于输出文件名的开头
        "input_file":"resources/test_csmar_news_24000_rows.pkl", # 输入文件，必选项，应该是一个pandas.DataFrame或内容是一个DataFrame的pickle文件的路径名
        ...
        "user_added_critic_words_filename":"resources/user_added_critic_words.txt", # 用户定义的关键词词表的路径名，可选项，默认为None，用于计算关键词的词频，，当且仅当only_retain_meaningful_words为True时生效，应该是一个以utf-8格式定义的每行一个词的文本文件的文件名
        ...
        "size_per_respawn_chunk_GB":0.02, # 存档块的大小，可选项，默认为0.6，应该是浮点数格式，每完成固定行数的分词会自动在respawnpoint文件夹中保存以实现断点续运行，在每一次循环中只将固定行数的文本读入内存，因此也与内存使用量有关，一个经验法则是1.5*内存大小（GB）/CPU逻辑处理器处理/待处理DataFrame的内存大小（GB）
